...
public:
    ...
    void OnWiFiModeChanged(ESPressio::WiFi::WiFiMode before, ESPressio::WiFi::WiFiMode after) override { if(!_output)return; Prefix(); _output->print("Mode "); _output->print(ModeName(before)); _output->print(" -> "); _output->println(ModeName(after)); }
    ...
    void OnScanStateChanged(ESPressio::WiFi::ScanState before,ESPressio::WiFi::ScanState after) override { if(!_output)return;Prefix();_output->print("Scan ");_output->print(ScanStateName(before));_output->print(" -> ");_output->println(ScanStateName(after)); }
    ...
    void OnClientIPAddressAcquired(const ESPressio::WiFi::NetworkAddress& n) override { if(!_output)return;Prefix();_output->print("ClientIPAddressAcquired ip=");_output->print(n.Address.ToString().c_str());_output->print(" gateway=");_output->println(n.Gateway.ToString().c_str()); }
...
private:
    ...
    void Station(const char* operation,const ESPressio::WiFi::MacAddress& mac){
        ...
        for(std::size_t i=0;i<mac.Octets.size();++i){if(i)_output->print(':');const uint8_t v=mac.Octets[i];_output->print(Hex(v>>4));_output->print(Hex(v&0x0F));}
        _output->println();
    }
